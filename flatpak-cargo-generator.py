...
CARGO_GIT_CRATES = f"{CARGO_HOME}/vendor-git"
VENDORED_SOURCES = "vendored-sources"
VENDORED_GIT_SOURCES = "vendored-git-sources"
        ...
        if "workspace" in item:
            if isinstance(workspace_item, dict):
                ...
                for dep_key, workspace_value in workspace_item.items():
                    ...
                    # Per Cargo docs, default-features and optional can be
                    # overridden by per-crate values in workspace inheritance.
                    elif dep_key in ("default-features", "optional") and dep_key in item:
                        pass  # keep per-crate value
                    else:
...
async def get_git_package_sources(
    ...
    cargo_vendored_entry: _VendorEntryType = {
        repo_url: {
            "git": repo_url,
            "replace-with": VENDORED_GIT_SOURCES,
        }
    }
    ...
    git_sources: List[_FlatpakSourceType] = [
        {
            "type": "shell",
            "commands": [
                f'mkdir -p "{CARGO_GIT_CRATES}" && cp -r --reflink=auto "{pkg_repo_dir}" "{CARGO_GIT_CRATES}/{name}"'
            ],
        },
        {
            "type": "inline",
            "contents": tomlkit.dumps(git_pkg.normalized),
            "dest": f"{CARGO_GIT_CRATES}/{name}",
...
async def generate_sources(
    ...
    cargo_vendored_sources = {
        ...
        VENDORED_GIT_SOURCES: {"directory": f"{CARGO_GIT_CRATES}"},
    }
